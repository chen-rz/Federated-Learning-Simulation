...
def get_dataloader(
        path_to_data, cid: str, is_train: bool, batch_size: int, workers: int
):
...
def do_fl_partitioning(path_to_dataset, pool_size, alpha, num_classes, val_ratio=0.0):
    ...
    # Save label distribution histograms to file
    with open("output/label_distribution_histograms.txt", mode='w') as outputFile:
